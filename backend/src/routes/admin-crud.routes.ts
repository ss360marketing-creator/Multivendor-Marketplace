  ...
  app.post(
    ...
    async (request, reply) => {
      const body = request.body as ProductBody & {
        image?: string
        images?: Array<{ url: string; altText?: string; sortOrder?: number; isPrimary?: boolean }> | string[]
        category?: string
        vendor?: string
      }
      ...
      const rawImages: Array<{ url: string; isPrimary: boolean }> = []
      ...
      if (Array.isArray(body.images)) {
        (body.images as unknown[]).forEach((img: unknown, idx: number) => {
          if (typeof img === 'string' && img.trim()) {
            const urlStr = img.trim()
            if (!rawImages.some(r => r.url === urlStr)) {
              rawImages.push({ url: urlStr, isPrimary: rawImages.length === 0 })
            }
          } else if (typeof img === 'object' && img !== null && 'url' in img && typeof (img as { url: unknown }).url === 'string') {
            const imgObj = img as { url: string; isPrimary?: boolean }
